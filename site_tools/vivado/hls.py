...
#-------------------------------------------------------------------------------
def generate_csynth_script(script_path, params, env):
    ...
    text += 'export_design -rtl verilog -format ip_catalog' + \
            ...
            '-vendor ' + params.vendor + ' -library ' + params.library + os.linesep*2
...
def hls_csynth(target, source, env):
    ...
    generate_csynth_script(csynth_script_path, params, env)
...
def launch_hls_csynth(env, src):
    ...
    for s in src:
        ...
        trg_name = get_name(s)
        target   = os.path.join(env['BUILD_HLS_PATH'], trg_name, trg_name + '.' + env['HLS_TARGET_SUFFIX'])
